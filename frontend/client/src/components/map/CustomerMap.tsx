...
// Convert map bounds to WKT polygon string
...
function CustomerMarkers({ customers }: { customers: Customer[] }) {
  const customersWithCoordinates = useMemo(() => {
  ...
  return (
    <>
      {customersWithCoordinates.map((customer) => {
        const coords = parseCoordinates(customer.coordinates);
        if (!coords) return null;
        
        return (
          <Marker key={customer.uuid} position={coords} icon={DefaultIcon}>
            <Popup>
              <div className="p-2">
                ...
                <p className="text-xs text-gray-500 mt-1">{customer.full_address}</p>
              </div>
            </Popup>
          </Marker>
        );
