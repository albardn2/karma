 ...
 * currency involved.
 */
export default function NewCustomersDashboard() {
  const { t } = useLanguage();
  const [gran, setGran] = useState<Gran>("month");

  const { data, isLoading, error } = useQuery<Payload>({
    queryKey: ["/dashboard/new-customers", gran],
    queryFn: () => apiRequest(`/dashboard/new-customers?granularity=${gran}`),
    retry: false,
  });
    ...
    <AppLayout>
      <div className="flex-1 overflow-auto p-4 lg:p-6 space-y-6">
        <div className="flex items-end justify-between flex-wrap gap-3">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{t("dashboards.newCustomers")}</h2>
            <p className="text-sm text-gray-600">{t("dashboards.newCustomersDesc")}</p>
          </div>
          {!forbidden && (
