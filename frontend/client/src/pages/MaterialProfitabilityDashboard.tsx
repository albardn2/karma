 ...
 */
/**
 * Revenue and gross margin per material for ONE period.
 *
 * Two modes, one component so the two can never drift apart:
 *   - no userUuid: business-wide, the management view, its own dashboard page.
 *   - userUuid:    that user's own orders, embedded on the user-analytics page
 *                  via the supervisory per-user endpoint.
 *
 * There is deliberately no `mine` variant. The other dashboards have one, but
 * this chart's cost column is warehouse LOT COST, which is not a rep's to see —
 * the backend has no /my-material-profitability for the same reason.
 */
export default function MaterialProfitabilityDashboard({
  embedded = false,
  userUuid,
}: {
  embedded?: boolean;
  /** Renders ONE user's margin — materials on orders that user created. Used
   *  embedded on the user-analytics page, where the page already names the
   *  person, so the heading drops to section size. */
  userUuid?: string;
}) {
  ...
  // Two payloads of the same shape from two endpoints: the per-user one is
  // supervisory-gated and takes a user_uuid, the business-wide one is the
  // management view. userUuid is in the key either way so the two never share
  // a cache entry.
  const endpoint = userUuid
    ? "/dashboard/user-material-profitability"
    : "/dashboard/material-profitability";
  const qs =
    `granularity=${gran}&offset=${offset}&target_currency=${ccy}` +
    (userUuid ? `&user_uuid=${userUuid}` : "");
  const { data, isLoading, error } = useQuery<Payload>({
    queryKey: [endpoint, userUuid ?? "", gran, offset, ccy],
  ...
  const forbidden =
    error != null && /^403/.test((error as Error).message || "");
  ...
  const effectiveMat =
    ppMaterials.length > 1 && ppMaterials.some((m) => m.uuid === ppMat)
      ? ppMat
      : ALL;
  ...
  return (
    // A userUuid always means embedded — it is only ever rendered inside the
    // user-analytics page — so a caller that forgets the flag cannot nest a
    // second AppLayout inside the first.
    <DashboardShell embedded={embedded || !!userUuid}>
      {/* As its own dashboard the title belongs above the card, the way every
          sibling page does it; embedded it stays in the card header, because
          the page around it already has a title of its own. */}
      {!userUuid && (
        <div>
          <h2 className="text-2xl font-bold text-gray-900">
            {t("dashboards.materialProfitability")}
          </h2>
          <p className="text-sm text-gray-600">
            {t("dashboards.materialProfitabilityDesc")}
          </p>
        </div>
      )}
      <Card
        data-testid={
          userUuid ? "user-material-profitability" : "material-profitability"
        }
      >
        ...
        <CardContent>
          ...
          <div className="mt-3 space-y-1 text-xs text-gray-500 dark:text-gray-400">
            ...
            {d && d.materials_omitted > 0 && (
              <p>
                {t("users.materialProfitabilityOmitted", {
                  count: d.materials_omitted,
                })}
              </p>
            )}
            {d && d.uncosted_quantity > 0 && (
              <p className="text-amber-600 dark:text-amber-500">
                {t("dashboards.uncosted", {
                  qty: fmtMoney(d.uncosted_quantity),
                })}
              </p>
            )}
            ...
            {d && d.uninvoiced_quantity > 0 && (
              <p className="text-amber-600 dark:text-amber-500">
                {t("users.materialUninvoiced", {
                  qty: fmtMoney(d.uninvoiced_quantity),
                })}
              </p>
            )}
            {d && d.unconverted_count > 0 && (
              <p className="text-amber-600 dark:text-amber-500">
                {t("dashboards.unconverted", {
                  amount: fmtMoney(d.unconverted_amount),
                })}
              </p>
            )}
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-xs uppercase tracking-wider text-gray-500">
                    ...
                    <th className="pb-2 pe-4 text-end">
                      {t("dashboards.revenue")}
                    </th>
                    <th className="pb-2 pe-4 text-end">
                      {t("users.materialCost")}
                    </th>
                    <th className="pb-2 pe-4 text-end">
                      {t("dashboards.gross")}
                    </th>
                    <th className="pb-2 text-end">
                      {t("users.materialMargin")}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {(data?.materials ?? []).map((m) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {m.revenue > 0
                          ? `${((m.gross / m.revenue) * 100).toFixed(1)}%`
                          : "—"}
                      </td>
                    </tr>
                  ))}
            ...
            >
                ...
                {ppMaterials.length > 1 && (
                  <Select value={effectiveMat} onValueChange={setPpMat}>
                    <SelectTrigger
                      ...
                      data-testid="ump-pp-material"
                    >
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value={ALL}>
                        {t("users.pricePointsAllMaterials")}
                      </SelectItem>
                      {ppMaterials.map((m) => (
              ...
              {effectiveMat === ALL && (d?.price_points_omitted ?? 0) > 0 && (
                <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                  {t("users.pricePointsOmitted", {
                    count: d!.price_points_omitted,
                  })}
                </p>
              )}
              {effectiveMat === ALL &&
