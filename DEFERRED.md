...
- **A year of USD→SYP history is not obtainable from sp-today.** Measured 2026-07-27: the public page embeds only the series behind its chart — **26 daily points, ~30 days** — and no query parameter extends it (`?period=1y`, `?range=365`, `?days=365` all return the same 26). There is no chart API to call; the page is server-rendered, so nothing else is fetched client-side. Archived snapshots don't rescue it: of 5 snapshots of that URL in the last year, only the newest (2026-07-22) predates nothing and carries a series (+6 usable days); the four older ones predate the site redesign and hold **a single day each** (e.g. 2026-01-20: buy 11,550 / sell 11,650 old SYP). Total obtainable ≈ 32 daily points + ~4 isolated samples. To get a real year, sp-today sells an API key that includes historical data (<https://sp-today.com/en/sptoday-api>) — wire `sp_today.py` to it once a key exists. **Small lift given a key**, impossible without one.

- **Nothing pulls the rate automatically.** `POST /exchange-rate/pull` is a button, so the history gains a point only when somebody clicks it — the table will gap on every day nobody does, which undermines the whole point of keeping rates over time. There is no scheduler in this repo today (no `schedule:` in any workflow, no worker process), so the cheapest honest option is a scheduled GitHub Actions job hitting the endpoint with a service token, or a cron on the droplet. **Small lift**, and it is the difference between a rate history and a rate scrapbook.
