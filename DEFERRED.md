...
- **The summary is web-only.** The Expo app's trip list has no multi-select and no aggregate; the app is the driver's tool and this is a supervisor's, so it was not carried over. Worth revisiting only if supervisors start auditing from phones.
